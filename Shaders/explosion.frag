...
void main()
{
	...
	f_color = ApplyFogColor(vec4((ambient + (diffuse + specular) * shadow + u_emissive), 1.0f));
	f_color.w = u_opacity;
	//f_color = vec4(v_screennormal, 1);
	brightcolor = min(vec4(min(pow(diffuse * shadow + max(u_emissive - vec3(1), 0), vec3(25)), 5), 1.0f), vec4(2, 2, 2, 1.0f));
	brightcolor += specular * shadow;
	brightcolor.w = u_opacity;
	if(u_ssao == 1)
	{
